...
import {
   ...
   Send, Edit2, RotateCcw, Archive, ChevronRight
} from 'lucide-react';
import { GlassCard } from './GlassCard';
import { Source, Hook, ActiveChat, ZoomCall, Seller } from '../types';
...
const SourcesStage: React.FC<{ sources: Source[]; totalNew?: number; onClick: () => void; onAddSource?: () => void }> = ({ sources, totalNew = 0, onClick, onAddSource }) => (
   <GlassCard className="h-full flex flex-col group cursor-pointer hover:border-neon-blue/30 transition-all" delay={0.1} onClick={onClick}>
      <div className="flex items-center justify-between mb-4">
         ...
         <div className="flex items-center gap-2">
            <span className="text-[10px] bg-neon-blue/10 text-neon-blue px-2 py-0.5 rounded border border-neon-blue/20 font-mono">
               {totalNew} NEW
            </span>
            <button
      ...
      <div className="mt-4 text-center text-[10px] text-slate-500 group-hover:text-neon-blue transition-colors flex items-center justify-center gap-1">
         Открыть все лиды <ChevronRight size={10} />
      </div>
   </GlassCard>
);
...
const HooksStage: React.FC<HooksStageProps> = ({ hooks, qualifiedCount = 0, contactedCount = 0, onClick, onAddHook, onHookUpdated }) => {
   ...
   const handleEditHook = async (hook: Hook) => {
      ...
      const newText = prompt(`Редактировать хук "${hook.name || hook.title}":\n\nТекущий текст:`, currentText);
   ...
   const handleArchiveHook = async (hook: Hook) => {
      setOpenMenuId(null);
      if (!confirm(`Архивировать хук "${hook.name || hook.title}"? Это деактивирует его.`)) return;
      try {
      ...
      <GlassCard className="h-full flex flex-col cursor-pointer hover:border-neon-purple/30 transition-all group" delay={0.2} onClick={onClick}>
         <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
               ...
               <span className="text-[10px] bg-neon-purple/10 text-neon-purple px-2 py-0.5 rounded border border-neon-purple/20">{qualifiedCount} QUAL</span>
               {contactedCount > 0 && (
                  <span className="text-[10px] bg-indigo-500/10 text-indigo-400 px-2 py-0.5 rounded border border-indigo-500/20">{contactedCount} SENT</span>
               )}
            </div>
         </div>
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-2">
                     ...
                     <div className="flex gap-2 relative">
                        ...
                        <button
                           ...
                           title={hook.active !== false ? 'Деактивировать' : 'Активировать'}
                        >
                           ...
                           {openMenuId === hook.id && (
                              ...
                              >
                                 <div className="p-1 space-y-0.5">
                                    ...
                                    <button
                                       onClick={() => handleResetStats(hook)}
                                       className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:text-white hover:bg-white/10 rounded flex items-center gap-2"
                                    >
                                       <RotateCcw size={12} /> Сбросить статы
                                    </button>
                                    <button
                                       onClick={() => handleArchiveHook(hook)}
         ...
         <div className="text-center text-[10px] text-slate-500 group-hover:text-neon-purple transition-colors flex items-center justify-center gap-1">
            Открыть кампанию <ChevronRight size={10} />
         </div>
      </GlassCard>
   );
};
...
const ChatsStage: React.FC<{ chats: ActiveChat[]; repliedCount?: number; onClick: () => void; onChatClick?: (chat: ActiveChat) => void }> = ({ chats, repliedCount = 0, onClick, onChatClick }) => (
   <GlassCard className="h-full flex flex-col cursor-pointer hover:border-yellow-500/30 transition-all group" delay={0.3} onClick={onClick}>
      <div className="flex items-center justify-between mb-4">
         <div className="flex items-center gap-2 text-yellow-400">
            <MessageSquare size={18} />
            <h3 className="font-semibold text-sm tracking-wider uppercase">Диалоги</h3>
         </div>
         <span className="bg-yellow-500/20 text-yellow-400 text-[10px] px-2 py-0.5 rounded animate-pulse border border-yellow-500/20">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-1">
         {chats.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-slate-600 text-xs py-8">
               ...
               <p className="text-[10px] text-slate-700 mt-1">Лиды появятся после отправки хуков</p>
            </div>
         )}
         {chats.map(chat => {
            ...
            return (
               <div
                  ...
                  className="p-3 rounded-xl bg-slate-900/40 border border-white/5 hover:bg-white/5 hover:border-neon-purple/30 transition-all cursor-pointer group/chat"
               >
                  <div className="flex justify-between items-center mb-2">
                     <h4 className="text-sm font-medium text-slate-200 group-hover/chat:text-white transition-colors">{chat.clientName}</h4>
                     <button
      ...
      <div className="mt-4 text-center text-[10px] text-slate-500 group-hover:text-yellow-400 transition-colors flex items-center justify-center gap-1">
         Все диалоги <ChevronRight size={10} />
      </div>
   </GlassCard>
);

const ZoomsStage: React.FC<{ zooms: ZoomCall[]; convertedCount?: number; onClick: () => void; onGenerateSummary?: (zoom: ZoomCall) => void }> = ({ zooms, convertedCount = 0, onClick, onGenerateSummary }) => {
   const approved = zooms.filter(z => z.status === 'confirmed');
   const pending = zooms.filter(z => z.status === 'pending');

   return (
      <GlassCard className="h-full flex flex-col cursor-pointer hover:border-neon-green/30 transition-all group" delay={0.4} onClick={onClick}>
         <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2 text-neon-green">
               <Video size={18} />
               <h3 className="font-semibold text-sm tracking-wider uppercase">Zoom & Продажи</h3>
            </div>
         ...
         {zooms.length === 0 && (
            <div className="flex flex-col items-center justify-center flex-1 text-slate-600 text-xs py-8">
               ...
               <p className="text-[10px] mt-1 text-slate-700">Лиды попадут сюда из диалогов</p>
            </div>
         )}
         ...
         <div className="flex-1 overflow-y-auto space-y-4 custom-scrollbar pr-1">
            ...
            {approved.length > 0 && (
               <div>
                  ...
                  <div className="space-y-2">
                     {approved.map(zoom => (
                        <div key={zoom.id} className="p-2.5 rounded-lg bg-neon-green/5 border border-neon-green/10" onClick={e => e.stopPropagation()}>
                           <div className="flex justify-between items-center mb-2">
                              <div>
                           ...
                           <button
                              onClick={(e) => { e.stopPropagation(); onGenerateSummary?.(zoom); }}
            ...
            {pending.length > 0 && (
               <div>
                  ...
                  <div className="space-y-2">
                     {pending.map(zoom => (
                        <div key={zoom.id} className="p-2.5 rounded-lg bg-white/5 border border-white/5 flex justify-between items-center opacity-70 hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                           <div>
         ...
         <div className="mt-4 text-center text-[10px] text-slate-500 group-hover:text-neon-green transition-colors flex items-center justify-center gap-1">
            Все зумы и продажи <ChevronRight size={10} />
         </div>
      </GlassCard>
   );
};
...
export const PipelineGrid: React.FC<PipelineGridProps> = ({
   sources, hooks, chats, zooms, sellers = [],
   qualifiedCount = 0, repliedCount = 0, convertedCount = 0,
   onStageClick, onAddSource, onChatClick, onGenerateSummary, onAddHook, onHookUpdated
}) => {
   const totalNew = sellers.filter(s => s.status === 'NEW').length || sources.reduce((sum, s) => sum + s.total - s.processed, 0);
   const contactedCount = sellers.filter(s => s.status === 'CONTACTED' || s.status === 'HOOK_SENT').length;

   return (
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3 h-full">
         <SourcesStage sources={sources} totalNew={totalNew} onClick={() => onStageClick('sources')} onAddSource={onAddSource} />
         <HooksStage
            hooks={hooks}
         ...
         <ZoomsStage zooms={zooms} convertedCount={convertedCount} onClick={() => onStageClick('zooms')} onGenerateSummary={onGenerateSummary} />
      </div>
   );
};
