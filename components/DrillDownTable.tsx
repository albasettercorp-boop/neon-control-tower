...
export const DrillDownTable: React.FC<DrillDownTableProps> = ({ view, onBack, projectId, sellers: sellersFromProps, isSandbox, onUpdateSellers, onAddHook, hooks }) => {
    ...
    const filteredSellers = sellers.filter(seller => {
        ...
        // Category filter (assuming seller has category field)
        if (filters.category !== 'all' && (seller as any).category !== filters.category) {
            return false;
        }

        // Health filter (assuming seller has health_status field)
    ...
    const fetchDetails = async () => {
        // Only fetch from DB if we are NOT using props (Real Mode)
        // or if props are empty for some reason
        if (sellersFromProps) {
            setLoading(false);
            return;
        }
    ...
    const handleProcessBatch = async () => {
        ...
        try {
            if (isSandbox && onUpdateSellers && sellersFromProps) {
                // Sandbox logic: Update local state without DB
                ...
                if (candidates.length === 0) {
                    console.warn("No candidates found for Sandbox processing");
                    setProcessing(false);
                    return;
                }
                ...
                const updatedSellers = sellersFromProps.map(s => {
                    if (candidateIds.has(s.id)) {
                        return { ...s, status: 'QUALIFIED', variant: Math.random() > 0.5 ? 'A' : 'B' }; // Assign variant locally
                    }
                    return s;
                });
                ...
                // 1. Find candidates (oldest first)
                let query = supabase
                    .from('sellers')
                    .select('id')
                ...
                if (fetchError || !candidates || candidates.length === 0) {
                    console.error("No candidates found or error", fetchError);
                    setProcessing(false);
                    return;
                }
    ...
    // Sync sellers from props when they change
    useEffect(() => {
        console.log('[DrillDownTable] useEffect triggered - sellersFromProps:', sellersFromProps?.length, 'view:', view);
        if (sellersFromProps) {
            setLoading(true);
            ...
            // Note: selectedSource filter is applied via filteredSellers, not here
            // This ensures all sellers from props are loaded first

            console.log('[DrillDownTable] Filtered sellers:', filtered.length, 'from', sellersFromProps.length);
            setSellers(filtered);
            setLoading(false);
        }
    }, [sellersFromProps, view]);

    // Fetch from Supabase only when no props and filters change
    useEffect(() => {
    ...
    // Sandbox: Simulate lead progression
    const handleSandboxAction = (sellerId: string, action: 'REPLY' | 'ZOOM' | 'CONVERT') => {
        ...
        const updatedSellers = sellersFromProps.map(s => {
            if (s.id === sellerId) {
                if (action === 'REPLY') {
                    return { ...s, status: 'REPLIED' };
                } else if (action === 'ZOOM') {
                    return { ...s, status: 'ZOOM_BOOKED' };
                } else if (action === 'CONVERT') {
                    return { ...s, status: 'CONVERTED' };
                }
            }
            return s;
        });
        ...
        <div className="h-full flex flex-col bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
            ...
            <div className="p-4 border-b border-white/10 flex flex-col gap-4 bg-white/5">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-2 border-t border-white/5">
                    ...
                    {/* Batch Actions (Only for Sources) */}
                    {view === 'sources' && (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="text-xs uppercase tracking-wider text-slate-400 border-b border-white/10 bg-white/5">
                                    ...
                                    <th className="p-3 font-medium text-left">Статус</th>
                                    {/* view === 'hooks' && <th className="p-3 font-medium text-left">Вариант</th>  -- Removed as hooks view is handled above */}
                            ...
                            <tbody className="text-sm">
                                {filteredSellers.map((seller, idx) => (
                                    ...
                                    >
                                        ...
                                        {/* Hooks variant removed from sellers view */}
                                        {isSandbox && (view === 'chats' || view === 'zooms') && (
