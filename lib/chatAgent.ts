...
export async function generateAgentResponse(
    ...
    const declineCount = conversationHistory.filter(
        (m) => m.role === 'user' && /(не интересно|не надо|спасибо нет|не актуально|не хочу)/i.test(m.content)
    ).length;

    if (!OPENAI_API_KEY) {
        console.warn('[ChatAgent] No VITE_OPENAI_API_KEY — using fallback mode');
    ...
    try {
        ...
        if (!response.ok) {
            console.error('[ChatAgent] OpenAI API error:', response.status, response.statusText);
            return fallbackResponse(userMessage, leadContext, messageCount, declineCount);
        }

        const data = await response.json();
        const agentMessage: string =
            data.choices?.[0]?.message?.content || fallbackResponse(userMessage, leadContext, messageCount, declineCount);
